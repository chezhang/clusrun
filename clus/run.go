...
func RunJob(headnode, command, sweep, output_dir, pattern string, groups, nodes []string, cache_size, prompt int, background, intersect bool) {
	...
	// Start job
	stream, err := c.StartClusJob(ctx, &pb.StartClusJobRequest{Command: command, Sweep: sweep, Pattern: pattern, Groups: groups, GroupsIntersect: intersect, Nodes: nodes}, grpc.UseCompressor("gzip"))
	if err != nil {
